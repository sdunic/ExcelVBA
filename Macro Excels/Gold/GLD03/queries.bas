...
Function selectPrices(objcint As String, arvcexr As String, cfin As String, barcodes As String) As String
    ...
    selectPrices = "EXEC [" + db.getDatabase + "].[" + db.getProcedurePrefix + "].[" + db.getProcedure + "] @objcint = N'" & objcint & "', @arvcexr = N'" & arvcexr & "', @cfin = N'" & cfin & "', @barcodes = N'" & barcodes & "'"
End Function
...
Function getInsertPrix(kodCjenika As String, datumCijene As Variant, staraCijena As String, novaCijena As String, sifraArtikla As String, cexv As String, kodPoreza As String, fich As String, valuta As String) As String
    
    datumPocetka = Application.Sheets(1).Range("C13").Value
    datumKraja = datumPocetka - 1
    
    Dim datumTrenutneCijene As Date
    If Len(datumCijene) > 0 Then
        datumTrenutneCijene = CDate(Split(datumCijene, " - ")(0))
    End If
    
    datumKrajaNoveCijene = "31.12.2049."
    If Len(Application.Sheets(1).Range("C15").Value) > 0 Then
        datumKrajaNoveCijene = Application.Sheets(1).Range("C15").Value
    End If
    
    If datumCijene > CDate("31-12-1899") And Len(novaCijena) > 0 Then
        getInsertPrix = getInsertPrix + queries.insertEndOfPrice(kodCjenika, utils.getDateString(datumTrenutneCijene), utils.getDateString(CDate(datumKraja)), sifraArtikla, Replace(staraCijena, ",", "."), cexv, kodPoreza, globals.getRowCount, utils.getDateString(Date), utils.getUserName, fich, globals.getRowNumber, valuta)
        globals.addRowNumber
        If novaCijena > 0 Then
            getInsertPrix = getInsertPrix + queries.insertNewPrice(kodCjenika, utils.getDateString(CDate(datumPocetka)), utils.getDateString(CDate(datumKrajaNoveCijene)), sifraArtikla, Replace(novaCijena, ",", "."), cexv, kodPoreza, globals.getRowCount, utils.getDateString(Date), utils.getUserName, fich, globals.getRowNumber, valuta)
            globals.addRowNumber
        End If
