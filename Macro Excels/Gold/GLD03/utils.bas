...
Sub setPrice(row As Long, dateCol As String, dateVal As Variant, endDateVal As Variant, priceCol As String, priceVal As Variant, newPriceCol As String, indexCol As String, priceId As Variant, changesCol As String)
   If Len(dateVal) > 0 Then
        dateVal = CDate(Replace(dateVal, " 00:00:00.0000000", ""))
        endDateVal = CDate(Replace(endDateVal, " 00:00:00.0000000", ""))
        Range(dateCol & row).Value = dateVal & " - " & endDateVal  'Datum
    End If
