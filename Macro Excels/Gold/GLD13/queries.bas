...
Function getInsertPrix(kodCjenika As String, datumCijene As Date, staraCijena As String, novaCijena As String, sifraArtikla As String, cexv As String, kodPoreza As String, fich As String, valuta As String) As String
    ...
    If datumCijene > CDate("31-12-1899") And Len(novaCijena) > 0 Then
        getInsertPrix = getInsertPrix + queries.insertEndOfPrice(kodCjenika, utils.getDateString(datumCijene), utils.getDateString(CDate(datumKraja)), sifraArtikla, Replace(staraCijena, ",", "."), cexv, kodPoreza, globals.getRowCount, utils.getDateString(Date), utils.getUserName, fich, globals.getRowNumber, valuta)
        globals.addRowNumber
        If novaCijena > 0 Then
            getInsertPrix = getInsertPrix + queries.insertNewPrice(kodCjenika, utils.getDateString(CDate(datumPocetka)), "31-12-2049", sifraArtikla, Replace(novaCijena, ",", "."), cexv, kodPoreza, globals.getRowCount, utils.getDateString(Date), utils.getUserName, fich, globals.getRowNumber, valuta)
            globals.addRowNumber
        End If
