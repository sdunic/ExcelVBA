...
Function loadDBResponse(msgid As String)
    loadDBResponse = "EXEC ('SELECT pkartcoca.get_closestEAN(123,arvcinv) EAN, pkstrucobj.get_desc(123,arvcinr,''HR'') NAZIV, INTSITE, INTQTEC, to_char(INTDCOM, ''dd-MM-yyyy''), to_char(INTDLIV, ''dd-MM-yyyy''), "
...
Function insertOrder(row As Long, msgid As String, seq As String)
    ...
    insertOrder = insertOrder + "INTPROPER) VALUES ("
        ...
        insertOrder = insertOrder + "NULL, " 'INTCOM1
        insertOrder = insertOrder + "NULL, " 'INTCOM2
        insertOrder = insertOrder + "0, " 'INTENLEV
        insertOrder = insertOrder + "trunc(sysdate), " 'INTDCOM
        insertOrder = insertOrder + "to_date(''" + CStr(Range(cfg.getcolINTDLIV & row).Value) + "'',''dd-mm-yyyy''), " 'INTDLIV
